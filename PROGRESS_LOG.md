...
>
> **NOW:** uninstall-feedback committed `f561252` + pushed → **CI build `28716153990` GREEN → standard APK DELIVERED** (`bannerlator-verified-uninstall-f561252-standard.apk`, sha head `3d4ca6b6a12f`), not yet installed. **DB cleaned to a clean slate for the repro** (removed orphaned HL2 queued `steam_downloads` row; FlatOut only installed; app force-stopped, backups kept). Device still runs the `52e7e38` build (vc37/2.2.2) until the new APK is installed. Steam/Goldberg already merged to main (`c89dc03`, no release, vc37/2.2.2). PARKED: Epic/GOG/Amazon registry wiring; bug (a) stale-byte-counter fix. NEXT: build green → deliver uninstall-feedback APK → user runs reinstall repro (with "Log debug session" ticked) on the clean slate → then merge decision. Detail in entries below + memory `project_bannerlator_download_manager`.
...
> - **✅ COMMITTED `f561252` (The412Banner) + pushed `feat/download-manager`. CI Build `28716153990` GREEN → standard APK DELIVERED** to `/sdcard/Download/bannerlator-verified-uninstall-f561252-standard.apk` (589,557,110 B, sha head `3d4ca6b6a12f`), media-scanned. **Not yet installed/device-tested.** User to install → test uninstall spinner+toast → run the reinstall repro (with "Log debug session" ticked) on the clean-slate DB.
>

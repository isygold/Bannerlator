...
> **→ NEW ACTIVE BRANCH `feat/download-manager-stores` (off main `ef717fb`):** route Amazon/GOG/Epic into the SAME `DownloadRegistry` as live producers, wired at the Activity callback seam (engines untouched). Order Amazon→GOG→Epic (lowest risk first). Recon plan done + saved to memory. **✅ Phase A (Amazon) BUILT — commit `bc6d27c`, CI `28719688616` RUNNING:** new `StoreDownloadHooks.kt` shim + `AmazonLibrarySync.kt` seeder + wired BOTH Amazon install entry points (detail + games-list) with real byte progress + `DownloadRegistry.init()`/seed in onCreate. On green → deliver APK → device-test (live row both paths, cancel clears, INSTALLED survives kill, uninstall clears). Then Phase B GOG. Parked: DepotSizeResolver true-size spec (memory).

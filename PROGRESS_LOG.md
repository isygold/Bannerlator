...
>
> **▶️ REMAINING PLAN (updated 2026-07-10, post-FEX-fixes):**
> **A. Last on-device verify before cutting a pre-release:** (1) **worker social WRITES** — tap Upvote → sticks+increments; post a comment → appears (reads already work; writes not yet confirmed hitting the worker). [Turnip driver + apply + component install now ✅ verified.]
> **B. Polish / loose ends:** cache-refresh affordance (index=in-mem+disk+24h → needs force-stop to see server updates); mark already-installed builds "Installed ✓" so they're not re-offered (harmless now); **check Box64/WOWBox64 gap** (smart-installer only auto-installs DXVK/VKD3D/FEXCore + Turnip — confirm box64/wowbox64 is handled/advisory); app_source label on cards; browser drill-position.
> **C. Track 3 = uploads (the "give back" half, next big feature):** export a Bannerlator shortcut → `pc_*` config (reverse `ConfigTranslator`) → `POST /upload` with `app_source="bannerlator"` (+ describe/delete own uploads) + local export/import. Zero new backend (worker already does uploads).
> **D. Ship:** cut **`2.6-preN`** once A done (B/C in later pre-releases).
> **E. Separate (not the feature):** CI **matrix** rework (prepare job + flavor matrix + `flavors: standard|all` input + `org.gradle.caching=true`) for build speed.
> **Recommended order:** A → cut 2.6-pre1 → C (uploads) as 2.6-pre2, folding B in.
>
> ---
